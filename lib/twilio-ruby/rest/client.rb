  ...
  module REST
    ...
    class Client
      ...
      def request(host, port, method, uri, params={}, data={}, headers={}, auth=nil, timeout=nil)
        whitelisted_uris = ["Messages.json", "Calls.json"]
        whitelisted_numbers = Twilio.whitelisted_numbers || []
        whitelisted_environments = Twilio.whitelisted_environments || []
        environment = Twilio.environment || "production"

        whitelisting_enabled = whitelisted_uris.filter{ |whitelisted_uri| uri.include?(whitelisted_uri) }.any?
        number_not_whitelisted = whitelisted_environments.include?(environment) && !whitelisted_numbers.include?(data["To"])

        if whitelisting_enabled && number_not_whitelisted
